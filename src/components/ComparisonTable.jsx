...
class ComparisonTable extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      compareItemsData: this.props.compareItemsData.sort((a, b) =>
        a['name'] > b['name'] ? 1 : -1,
      ),
    ...
    if (!activeStatsHeaders[itemStat].isSelected) {
      ...
      this.setState(prevState => {
        return {
          ...
          compareItemsData: prevState.compareItemsData.sort((a, b) =>
            a[itemStat.replace(' ', '')] > b[itemStat.replace(' ', '')]
              ? 1
              : -1,
          ),
        }
        ...
        <thead>
          <tr>
            ...
            <th
              ...
              style={{ display: mobileSize ? 'none' : 'table-cell' }}
            >
              Image
            </th>
              ...
              <th
                ...
                className={
                  activeStatsHeaders[header].isSelected
                    ? activeStatsHeaders[header].isAscending
                      ? 'stat-active ascending'
                      : 'stat-active descending'
                    : null
                }
                key={index}
                onClick={() => this.handleClickSortBy(header)}
              >
                <StatDisplay statType="Table Headers" statName={header} />
              </th>
            ))}
          </tr>
        </thead>
...
ComparisonTable.propTypes = {
  activeStatsHeaders: PropTypes.object.isRequired,
  compareItemsData: PropTypes.object.isRequired,
  itemOrderAscending: PropTypes.bool.isRequired,
  mobileSize: PropTypes.bool.isRequired,
  statsToSort: PropTypes.instanceOf(Set).isRequired,
}
