...
class DisplayTopCategoryItem extends Component {
  constructor(props) {
    ...
    this.state = {
      topCategoryItem: this.props.compareItemsData.sort((a, b) => (a["damage"] < b["damage"]) ? 1 : -1)[0],
      categories: {
        "damage": {
          "isSelected": true,
          "topItem": {}
  ...
  handleEvent = (e, clickedButton) => {
    ...
    if (((clickedButton === "leftButton") || (e.type === 'keydown' && e.key.toLowerCase() === 'q'))
      ...
      copyOfCategories[nextCategoryName].topItem = this.props.compareItemsData.sort((a, b) => (
        a[nextCategoryName.replace(' ', '')] < b[nextCategoryName.replace(' ', '')])? 1 : -1)[0]
      
      this.setState((prevState) => {
        return {
          topCategoryItem: copyOfCategories[nextCategoryName].topItem,
          currCategoryIndex: currCategoryIndex - 1,
          categories: copyOfCategories
        }
      })
    }
    else if (((clickedButton === "rightButton") || (e.type === 'keydown' && e.key.toLowerCase() === 'e'))
      ...
      <div className="nav-wrapper">
        ...
        <Level className="category-nav">
            <Level.Item className="nav-button"> 
              <button onClick={(e) => this.handleEvent(e, "leftButton")}>
                Q
              </button>
            </Level.Item>
          <Level.Item className="category-name">
            ...
            <Level.Item className="nav-button">
              <button onClick={(e) => this.handleEvent(e, "rightButton")}>
                E
              </button>
            </Level.Item>
        </Level>
        ...
        <Level className="category-nav-circles">
          {
            ...
            Object.keys(categories).map((category, index) => 
              ...
              <Level.Item key={index} className={categories[category].isSelected ? "category-circle circle-active" : "category-circle"}>
